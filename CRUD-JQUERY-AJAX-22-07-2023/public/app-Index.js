...
var saveData = (event) => {
    ...
    if(document.querySelector(".emailExist").style.display == "block") {
        $(".mainValidation").show();
        return false;
    } else {
        $(".mainValidation").hide();
    ...
    if (fileInfo == undefined) {
        ...
        $.ajax({
            ...
            $("#email").val("");
            $("#mob_no").val("");
            $("#post").val($("#post option[selected]").val());
            $("#bld_group").val($("#bld_group option[selected]").val());
            $("#date").val("");
